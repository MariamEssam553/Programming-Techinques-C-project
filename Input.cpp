...
{	
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		...
		{	
			...
			case ITM_PAST_CARD: return PASTE_CARD;
			case ITM_EDIT_CARD: return EDIT_CARD;
			case ITM_DELETE_GAME_OBJECT: return DELETE_GAME;
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		...
		{
			int xcelll;
			xcelll = x / UI.CellWidth;
			int ycelll;
			ycelll = (y - UI.ToolBarHeight) / UI.CellHeight;
			cellPos.SetHCell(xcelll);
			cellPos.SetVCell(ycelll);
			cellPos.GetCellNum();
		}
		else
		{
			CellPosition cellpooos;
			return cellpooos;
		}
	}
	return cellPos;
